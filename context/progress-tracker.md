...
* **Hero Banner Redesign, Graphic Carousels, Sidebar Reduction & UI Rules** (`components/parent/hero-banner.tsx`, `components/shell/desktop-sidebar.tsx`, `components/shell/application-shell.tsx`, `components/shell/primary-navigation.tsx`, `components/shell/mobile-header.tsx`, `components/shell/shell-logo.tsx`, `components/shell/footer-region.tsx`, `context/ui-context.md`, `context/design-system.md`) — Completed 100%. 
  ...
  3. **Auto-Sliding Graphic Banner Carousel with Infinite Loop**: Built a 4.5-second auto-rotating, touch-swipeable graphic carousel with dot pagination indicators, uniform fixed height (`h-[140px] sm:h-[145px]`), restored `bg-gradient-to-br` background gradients, and an extended circular track loop (`[...slides, ...slides]`) that peeks slide 1 on the right when slide 3 is active with zero blank space:
     - Slide 1: `Get ₦300 when you sign up` with highlighted `₦300` text + yellow gift box graphic (`/gift-box.png`) floating flush at `bottom-0 right-0`.
     - Slide 2: `Don't miss out on Sunday Evictions` with highlighted `Sunday Evictions` text + 3D clock graphic (`/clock.png`) sitting flush at `bottom-0 right-0`.
     - Slide 3: `Trade your opinion for profit` with highlighted `opinion` text + 3D Sheybi mascot graphic (`/sheybi-mascot.png`) sitting flush at `bottom-0 right-0`.
