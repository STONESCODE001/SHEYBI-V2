...
* **Default ₦300 Signup Bonus for All New User Registrations** (`lib/actions/wallet-provisioning.ts`) — Completed 100%. Updated `ensureUserWalletAction()` so that all brand-new user signups (direct registrations without a referral link) automatically receive the advertised **₦300 Playable Bonus** in their InstantDB wallet (`availableBalance += 300`, `bonusBalance += 300`) upon initial wallet provisioning, marking `signupBonusAmount: 300` and `signupBonusClaimed: true` on `$users` and creating transaction/ledger records. Verified with zero TypeScript errors (`npx tsc --noEmit`).

* **1-Click Admin Email Broadcast for New Markets** (`lib/actions/email-actions.ts`, `components/admin/admin-markets-tab.tsx`, `app/admin/page.tsx`) — Completed 100%. (1) Created `broadcastMarketEmailAction()` in `lib/actions/email-actions.ts` using zero-dependency native HTTP `fetch` to trigger Loops API (`https://app.loops.so/api/v1/transactional`) with your published `LOOPS_MARKET_ALERT_ID` (`cmu2qpdcw0axh0jz3cvyxb3ve`), dynamically passing `{marketTitle}` and `{marketUrl}`; (2) Added a high-contrast indigo **"Send Alert"** button (`<Mail className="h-3.5 w-3.5" />`) next to every live market in `AdminMarketsTab`; (3) Wired `handleBroadcastMarketEmail` in `app/admin/page.tsx` with Sonner toast feedback ("*Email alert broadcasted to X registered users!*"). Verified with zero TypeScript errors (`npx tsc --noEmit`).
...
* **Hero Banner Redesign, Graphic Carousels, Sidebar Reduction & UI Rules** (`components/parent/hero-banner.tsx`, `components/shell/desktop-sidebar.tsx`, `components/shell/application-shell.tsx`, `components/shell/primary-navigation.tsx`, `components/shell/mobile-header.tsx`, `components/shell/shell-logo.tsx`, `components/shell/footer-region.tsx`, `context/ui-context.md`, `context/design-system.md`) — Completed 100%. 
  ...
  3. **Responsive Mobile Full-Width & Desktop 2-Slot Circular Reel**: Updated [`components/parent/hero-banner.tsx`](file:///c:/Users/THE%20LAPTOP%20STORE/Desktop/SHEYBI%20FAMILY/SHEYBI-V2/components/parent/hero-banner.tsx) so mobile renders a **100% full-width single banner card** (`w-full`) per slide that auto-slides seamlessly to the next full-width banner, while desktop displays the 2-slot circular reel (Main ~64% + Peek ~36% queued behind it):
     - **Mobile**: Full width `w-full h-[140px] sm:h-[145px]` banner with rich gradient background, title copy, highlighted keywords, floating graphic, touch swipe, and 4.5s auto-slide to the next full-width banner.
     - **Desktop**: 2-slot Main + Peek Reel where Slot 2 peeks the next banner queued seamlessly behind Main (Banner 3 peeks Banner 1).
     - **Zero Blank Space & Zero Duplicates**: Infinite loop (1 → 2 → 3 → 1) with zero empty whitespace and zero duplicate banners.
  4. **Graphic Scaling & Containment**: Constrained bottom-right floating images to `max-h-[120px] sm:max-h-[130px]` and `w-28 sm:w-36 md:w-40` with `object-contain object-bottom` to prevent image overflow clipped bounds.
  5. **Desktop Sidebar Width Reduction**: Reduced desktop sidebar width from `260px` to **`220px`** (`w-[220px]` in `desktop-sidebar.tsx` and `lg:pl-[220px]` offset in `application-shell.tsx`), freeing up 40px horizontal space for main feed/content grids.
  6. **Clean Sidebar Navigation Styling**: Stripped the yellow left side border (`border-l-4 border-l-[#FFC91F]`) from active navigation buttons in `primary-navigation.tsx`, styling active state as clean elevated surface pills (`bg-[#1E2A44] text-white font-bold shadow-sm`).
  7. **UI Context Rule Extensions (`context/ui-context.md`)**: Added Rule #11 (forbidding raw emoji characters in UI copy) and Rule #12 (enforcing clean, borderless active navigation states). Verified with zero TypeScript errors (`npx tsc --noEmit`).

* **Next.js 16 Proxy Migration & Clerk Auth Deprecation Cleanup** (`proxy.ts`, `context/feature-specs/08-realtime-authentication-spec.md`, `context/feature-specs/21-promoter-referral-tracking.md`) — Completed 100%. (1) Migrated root `middleware.ts` to `proxy.ts` conforming to Next.js 16 file convention; (2) Removed deprecated Clerk `createRouteMatcher` and replaced with explicit string prefix matching (`PROTECTED_PREFIXES` and `ADMIN_PREFIXES`); (3) Preserved referral link cookies (`/f/[promoter]` and `?ref=`) and admin role protection checks; (4) Updated system documentation specs. Verified with zero TypeScript errors (`npx tsc --noEmit`) and successful Next.js production build (`npm run build`).

* **UI Visual Audit & CTA Visual Hierarchy Refinement** (`components/shell/wallet-chip.tsx`, `components/shell/primary-navigation.tsx`, `components/shell/mobile-header.tsx`, `components/shell/user-profile-region.tsx`, `components/parent/category-tabs.tsx`, `components/parent/market-card.tsx`, `components/parent/market-details/versus-market-view.tsx`, `components/parent/market-details/binary-market-view.tsx`, `components/parent/market-details/multi-option-market-view.tsx`) — Completed 100%. (1) Comprehensive page visual audit executed to eliminate visual noise and focus user attention; (2) Reserved bright golden yellow (`#FFC91F`) exclusively for primary prediction CTAs (**`Predict Now →`**); (3) Subdued Header Wallet Chip from solid yellow block to sleek dark surface pill (`bg-[#0F1727] border border-white/10 text-[#FFC91F] font-mono font-bold`); (4) Subdued Desktop Sidebar active nav tab from solid yellow rectangle to elevated dark surface (`bg-[#162032] text-white border-l-4 border-l-[#FFC91F] font-bold`) and `+` FAB to neutral border; (5) Subdued Category Filter active tabs ("Trending") from bright golden yellow pill (`bg-[#FFC700] text-black`) to dark elevated pill (`bg-[#162032] text-white border border-white/20`); (6) Subdued header/sidebar guest "Sign up" buttons to primary indigo (`bg-indigo-600 text-white`); (7) Subdued `← Back` links and explore buttons on detail views to clean neutral muted slate styling. Verified with zero TypeScript errors (`npx tsc --noEmit`) and successful Next.js production build (`npm run build`).

* **Admin Draft Market "Go Live" Publishing** (`lib/actions/market-actions.ts`, `components/admin/admin-markets-tab.tsx`, `app/admin/page.tsx`) — Completed 100%. (1) Created `publishMarketAction()` server action in `lib/actions/market-actions.ts` validating admin authentication via Clerk, updating market state from `'draft'` to `'open'`, creating a `market_activity` record (`"Market published live by admin"`), and recording an audit log entry (`PUBLISH_MARKET`); (2) Updated `AdminMarketsTab` in `components/admin/admin-markets-tab.tsx` to display a high-contrast green **"Go Live"** action button (`<Play />`) for all markets in `Draft` status and restrict resolution actions to non-draft markets; (3) Wired `handlePublishMarket` in `app/admin/page.tsx` with Sonner toast notification feedback ("*Market is now live!*"). Verified with zero TypeScript errors (`npx tsc --noEmit`) and clean production build pass (`npm run build`).
...
* **1v1 Market UX & Clarity Enhancements** (`components/child/odds-button.tsx`, `components/parent/market-card.tsx`, `components/parent/market-details/versus-market-view.tsx`) — Completed 100%. 
  ...
  3. **1v1 Market Detail Page Enhancements (`versus-market-view.tsx`)**: 
     - **Master Hint Banner**: Styled with soft surface card (`bg-[var(--surface-subtle)]` with `border-[var(--border-default)]`) featuring Lucide `HelpCircle` icon in Primary Indigo (`text-[var(--primary)]`): `How 1v1 Predictions Work: Pick YES if you think that candidate will win, or NO if you think they will lose.`
     ...
     - **Dual Micro Info Banners**: Added dedicated micro hint banners directly under **both** Contestant 1 and Contestant 2 action blocks featuring Lucide `HelpCircle` icon in Primary Indigo (`text-[var(--primary)]`): `<HelpCircle /> YES = [Name] wins | NO = [Name] loses`.
