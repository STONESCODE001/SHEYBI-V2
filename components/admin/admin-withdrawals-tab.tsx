            ...
            <tbody className="divide-y divide-border">
              {withdrawals.length === 0 ? (
                ...
                withdrawals.map((item) => (
                  <tr key={item.id} className="hover:bg-surface-bright/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="font-semibold text-xs text-text-primary">{item.bankName}</div>
                      <div className="text-xs font-mono text-text-secondary">{item.accountNumber}</div>
                    </td>
                    <td className="px-4 py-3 font-bold text-text-primary">
                      {formatNaira(item.amount)}
                    </td>
                    <td className="px-4 py-3 text-xs text-text-muted">{item.requestDate}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-right">
                      {item.status === "Pending" ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg bg-primary text-on-primary px-3 py-1.5 text-xs font-semibold hover:bg-primary-hover shadow-xs transition-all"
                        >
