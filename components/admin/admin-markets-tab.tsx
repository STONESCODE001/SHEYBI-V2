    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 items-center gap-2">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full rounded-xl border border-border bg-surface-bright pl-9 pr-4 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-primary focus:outline-none"
            />
          </div>
          ...
          <select
            ...
            className="rounded-xl border border-border bg-surface-bright px-3 py-2 text-sm text-text-primary focus:border-primary focus:outline-none"
          >
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 rounded-xl bg-primary text-on-primary px-4 py-2.5 text-sm font-semibold hover:bg-primary-hover shadow-sm transition-all"
        >
      ...
      {/* Markets Table */}
      <div className="overflow-hidden rounded-xl border border-border bg-surface-subtle shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-text-primary">
            <thead className="border-b border-border bg-surface-container/60 text-xs uppercase text-text-muted">
              <tr>
            ...
            <tbody className="divide-y divide-border">
              {filteredMarkets.length === 0 ? (
                ...
                filteredMarkets.map((market) => (
                  <tr key={market.id} className="hover:bg-surface-bright/50 transition-colors">
                    <td className="px-4 py-3 font-medium">
                      <div className="font-semibold text-text-primary">{market.title}</div>
                      <div className="text-xs text-text-muted">Closes: {market.closeDate}</div>
                    </td>
                    <td className="px-4 py-3 text-text-secondary capitalize">{market.category}</td>
                    <td className="px-4 py-3 text-xs uppercase font-medium text-text-muted">
                      {market.format}
                    </td>
                    ...
                    <td className="px-4 py-3 font-medium">{formatNaira(market.totalVolume)}</td>
                    <td className="px-4 py-3 text-right">
                      {market.status === "Closed" ? (
                        ...
                        <span className="text-xs text-text-muted italic">Live Trading</span>
                      ) : (
